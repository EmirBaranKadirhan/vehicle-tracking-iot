...
// Her araç için renk ve isim tanımları
const VEHICLE_COLORS: Record<string, { color: string; label: string }> = {
  '1': { color: '#06b6d4', label: 'Alpha-1' },  // cyan
  '2': { color: '#a855f7', label: 'Alpha-2' },  // purple
  '3': { color: '#f59e0b', label: 'Alpha-3' },  // amber
}

export default function ObsidianDashboard() {
  // vehicles => { '1': { lat, long, speed, ... }, '2': { ... }, '3': { ... } }
  const [vehicles, setVehicles] = useState<Record<string, IGPSData>>({})
  const [history, setHistory] = useState<Array<IGPSData & { id: string }>>([])
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      // vehicles objesinde ilgili aracı güncelle, diğerlerini koru
      setVehicles(prev => ({ ...prev, [id]: gpsData }))

      // Geçmişe ekle, id'yi de tut
      setHistory(prev => [{ id, ...gpsData }, ...prev].slice(0, 10))
    }
    ...
    <div className="flex h-screen overflow-hidden bg-[#0b1326] text-[#dae2fd] font-['Inter']">
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        ...
        <div className="p-8 space-y-8">
          ...
          {/* Her araç için Speed Gauge + StatCard grubu */}
          {Object.entries(vehicles).map(([id, vehicle]) => {
            const colorInfo = VEHICLE_COLORS[id]
            return (
              <div key={id} className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-bold font-['Space_Grotesk']" style={{ color: colorInfo?.color }}>
                    {colorInfo?.label}
                  </h3>
                </div>
                ...
                <div className="grid grid-cols-12 gap-4">
                  ...
                  <div className="col-span-12 md:col-span-3 bg-[#131b2e] p-6 rounded-2xl border border-white/5">
                    ...
                    <h3 className="text-2xl font-black font-['Space_Grotesk'] text-emerald-400">
                      {vehicle.speed > 0 ? 'Moving' : 'Idle'}
                    </h3>
                  </div>
                </div>
              </div>
